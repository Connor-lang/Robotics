...
#loads pretrained face detector and mask detector model 
...
while True:
    frame = vs.read() #reads the current frame 

    image_height, image_width, _ = frame.shape
    blob = cv2.dnn.blobFromImage(frame, scalefactor=1.0, size=(300, 300),
                                mean=(104.0, 117.0, 123.0), swapRB=False, crop=False)
    faceNet.setInput(blob)
    results = faceNet.forward() #pass the frame into neural network for processing 
    ...
    for face in results[0][0]:
        ...
        if face_confidence > args["confidence"]: 
            #gets the coordinate of faces detected with confidence than the set confidence (0.5)
            bbox = face[3:]
    ...
    if len(faces) > 0:
        for face in faces:  
            preds.append(maskNet.predict(face)) #predicts whether the face has a mask; results are appended to a list
    
    for (box, pred) in zip(locs, preds):
        (startX, startY, endX, endY) = box #draws the bounding box of the faces
        (mask, withoutMask) = pred[0] #confidence for mask and withoutMask 
        label = "Mask" if mask > withoutMask else "No Mask"
        color = (0, 255, 0) if label == "Mask" else (0, 0, 255)
        label = "{}: {:.2f}%".format(label, max(mask, withoutMask) * 100)
        #labels the bounding box with face-mask on or not on 
        if mask > withoutMask:
